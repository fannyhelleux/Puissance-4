...
public class Partie {
    ...
    public void debuterPartie(){
        ...
        while(grille.etreGagnantePourJoueur(listeJoueurs[0])==false||grille.etreGagnantePourJoueur(listeJoueurs[1])==false||grille.etreRemplie()==false){
            ...
            System.out.println("\nQue veut tu faire ?\n");
            if (miftah==5){
                System.out.println("What do u wanna do ?? (désolé y'a miftah qui s'est introduit dans le code, je m'occupe de lui, \"allez miftah on y gow\")\n");
                miftah=0;
            }
            miftah++;
            ...
            action = sc.nextLine(); // visiblement cette instruction se fait sauter 1 fois sur deux, c'est peut etre une demi pute...
            ...
            if ("d".equals(action)){
                ...
                if (joueurCourant.utiliserDesintegrateur()==true){
                   ...
                   System.out.println("Tu n'as que chi en terme de désintégrateurs, choisis une autre action !");
                }
            }
            ...
            if ("r".equals(action)){
                if (joueurCourant.listeJetons.length<20){
                    System.out.println("Quel jeton veux-tu bien récuperer?");
                    System.out.print("ligne : ");
                    int ligneD= sc.nextInt()-1;
                    System.out.print("colonne : ");
                    int colonneD= sc.nextInt()-1;
                    ...
                    while ((joueurCourant.couleur).equals(grille.cellules[ligneD][colonneD].jetonCourant.couleur)==false || grille.cellules[ligneD][colonneD].supprimerJeton()== false){
                        System.out.println("Tu ne peux guère récuperer de jeton ici \nQuel est donc le jeton que tu veux bien récuperer?");
                        System.out.print("ligne : ");
                        ligneD= sc.nextInt()-1;
                        System.out.print("colonne : ");
                        colonneD= sc.nextInt()-1;
                    ...
                    System.out.println("Toutes mes excuses, mais tu n'as guère de jetons à récupérer...");
                }
            }
            ...
            if ("j".equals(action)){
                if (joueurCourant.listeJetons.length>0){
                    System.out.println("Où veux-tu donc placer ton jeton ?");
                    System.out.print("colonne : ");
                    ...
                    while (grille.ajouterJetonDansColonne(joueurCourant.listeJetons[i],colonnej)==false){
                        System.out.println("Tu ne peux guère placer de jeton ici \nOù veux-tu bien placer ton jeton ?");
                        System.out.print("colonne : ");
                        colonnej= sc.nextInt()-1;
                    ...
                    if (grille.cellules[lignej][colonnej].presenceDesintegrateur()){
                        ...
                        System.out.println("Tu as réussi à te dégoter un nouveau désintégrateur!");
                    }
                    if (grille.cellules[lignej][colonnej].presenceTrouNoir()){
                        ...
                        System.out.println("Tu as activé un trou noir... LOOOSSERRR");
                    }
                    compteur=1;
                }
                else{
                    System.out.println("Tu n'as plus de jetons... Choisis une autre action! Another one ! (désolé c'est Khaled qui prends la conf)");
                }
            }
            if ("q".equals(action)){
                break;
